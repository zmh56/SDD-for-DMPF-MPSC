...
class SincNet_attention_gnn(nn.Module):

    def __init__(self,options):
        super(SincNet_attention_gnn,self).__init__()
        ...
        self.mulhead_num_hiddens = options['mulhead_num_hiddens']
        self.mulhead_num_heads = options['mulhead_num_heads']
        self.mulhead_num_query = options['mulhead_num_query']
        self.dropout_fc = options['dropout_fc']
        self.hidden_dims_fc = options['hidden_dims_fc']
        self.att_hidden_dims_fc = options['att_hidden_dims_fc']
        ...
        self.out_dim = current_input * self.mulhead_num_hiddens

        self.mul_attention = multi_head.MultiHeadAttention(self.mulhead_num_query, self.mulhead_num_query,
                                                           self.mulhead_num_query,
                                                           self.mulhead_num_hiddens, self.mulhead_num_heads, 0.5)

        self.fc_audio1 = nn.Sequential(
            nn.Linear(self.out_dim, self.att_hidden_dims_fc),
            nn.BatchNorm1d(self.att_hidden_dims_fc, momentum=0.05),
            nn.LeakyReLU(0.2),
            nn.Dropout(self.dropout_fc),
            nn.Linear(self.att_hidden_dims_fc, self.hidden_dims_fc),
        )

        self.fc_audio2 = nn.Sequential(
            nn.BatchNorm1d(self.hidden_dims_fc, momentum=0.05),
            nn.LeakyReLU(0.2),
            nn.Dropout(self.dropout_fc),
            nn.Linear(self.hidden_dims_fc, self.num_classes),
        )
    ...
    def forward(self, x):
        ...
        x = x.permute(0,2,1)
        x = self.mul_attention(x, x, x, valid_lens=None)
        x = x.view(batch, -1)
